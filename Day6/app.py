...
from test_api import Test, Song
api.add_resource(Test, '/api/test_x')
api.add_resource(Song, '/api/songs')
...
@user_required()
def becomeCreator():
    curr_user = get_current_user() # <u>
    curr_user.isCreator = True      # <u>.isCreator=True
...
def remove_from_playlist():
    ...
    #get Id
    playlist_id = request.args.get('playlist_id') # 2
    song_id = request.args.get('song_id') # 1

    #get Obj/row
    playlist = Playlist.query.filter_by(id=playlist_id).first() # <p2>
    song = Song.query.filter_by(id=song_id).first() # <s1>
...
def admin_blacklist_creator():
    ...
    # int id ==> userobj
    creator_id = request.args.get('creator_id') # 3
    creator = User.query.filter_by(id=creator_id, isCreator=True).first() # <user3>

    # userobj.isBlacklisted = True
    creator.isBlacklisted = True # <user3>.isBlacklisted=True
    db.session.commit() # db.session.commit()
...
def admin_whitelist_creator():
    ...
    creator_id = request.args.get('creator_id') # 3
    creator = User.query.filter_by(id=creator_id, isCreator=True).first() # <u3>
    if creator:
